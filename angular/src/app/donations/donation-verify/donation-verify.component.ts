...
@Component({
  selector: 'app-donation-verify',
  templateUrl: './donation-verify.component.html',
  styleUrls: ['./donation-verify.component.scss'],
  standalone: true,
  imports: [
    CommonModule, 
    ReactiveFormsModule,
...
export class DonationVerifyComponent implements OnInit {
  donation?: DonationDto;
  verifyForm!: FormGroup;
  loading = false;
  donationId?: string;
  mode: 'create' | 'verify' | 'edit' = 'create';
  ...
  editorConfig = {
    ...
    toolbar: 'undo redo | blocks fontsize | bold italic underline forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist | table link | print | removeformat fullscreen',
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      if (!this.donationId) {
        ...
        this.resetFormData();
        // Has ID - load donation and determine if VERIFY or EDIT
        this.loadDonation();
  ...
  loadDonation(): void {
    ...
    this.donationService.get(this.donationId).subscribe({
      next: (donation) => {
        ...
        // Determine mode based on donation status
  ...
  verify(): void {
    ...
    const dto = {
      ...
      // Include rendered letter content if template was selected and edited
      renderedLetterContent: this.selectedTemplateId ? this.letterContent : null
    };
  ...
  goToFirst(): void {
    if (this.filteredDonationIds.length > 0) {
      ...
      if (this.mode === 'create') {
        ...
        this.router.navigate(['/donations/verify', firstId], {
  ...
  goToPrevious(): void {
    if (this.currentIndex > 0) {
      ...
      if (this.mode === 'create') {
        ...
        this.router.navigate(['/donations/verify', prevId], {
          queryParamsHandling: 'preserve'
        });
      } else {
  ...
  goToNext(): void {
    if (this.currentIndex < this.filteredDonationIds.length - 1) {
      ...
      if (this.mode === 'create') {
        ...
        this.router.navigate(['/donations/verify', nextId], {
          queryParamsHandling: 'preserve'
        });
      } else {
  ...
  goToLast(): void {
    if (this.filteredDonationIds.length > 0) {
      ...
      if (this.mode === 'create') {
        ...
        this.router.navigate(['/donations/verify', lastId], {
          queryParamsHandling: 'preserve'
        });
      } else {
